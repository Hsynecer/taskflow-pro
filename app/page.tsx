...
export default function Home() {
  ...
  // DİKKAT: Artık 'columns' dizisini ve 'initializeBoard' fonksiyonunu store'dan çekiyoruz
  ...
  // 2. GÜVENLİK VE AUTO-PROVISIONING (Bilet kontrolü ve Panoyu Hazırlama)
  useEffect(() => {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    // Görev boş bir sütunun üzerine bırakıldıysa (Artık dinamik 'columns' içinde arıyoruz)
  ...
  // Güvenlik ve Veri Yükleme Ekranı
    ...
    <main className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-7xl mx-auto">
        <header className="mb-10 text-center">
          ...
          <p className="text-gray-500 mt-2">KoçSistem Teknik Projesi</p>
        </header>

        <DndContext collisionDetection={closestCorners} onDragEnd={handleDragEnd}>
          <div className="flex gap-6 justify-center overflow-x-auto pb-4">
            {/* SÜTUNLARI DİNAMİK OLARAK RENDER EDİYORUZ */}
            {columns.map((column) => (
              <Column
                key={column.id}
                columnId={column.id}
